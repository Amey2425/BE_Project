...
TRAIN = os.getenv("CLIENT_1_TRAINING_SET")
TEST = os.getenv("CLIENT_1_TESTING_SET")


class FlowerClient(NumPyClient):
    def __init__(self):
        self.model = NeuralNetworkAlgo()
        self.trainset = TRAIN
        self.testset = TEST
    ...
    def fit(self, parameters, config=None):
        ...
        _, y = self.model.load_data(self.trainset)
        return self.model.get_weights(), len(y), {}

    def evaluate(self, parameters, config=None):
        self.model.set_weights(parameters)
        loss, acc = self.model.test(self.testset)
        _, y = self.model.load_data(self.testset)
